...
import styles from "./PixelLiquidBg.module.css";
...
/*
  ...
  Ported from ui.unlumen.com/components/pixel-liquid-bg. Two changes for this
  codebase: Tailwind/`cn` are swapped for a CSS module, and the light/dark
  palette pair is collapsed into a single `palette` (the site has no dark mode).
  Palette stops are also parsed as raw sRGB bytes rather than via THREE.Color —
  RawShaderMaterial output is not colour-managed, so raw bytes are what makes
  the rendered colours match the brand hexes exactly.
*/
...
/** "#2fa1e0" | "#2fa" -> [r, g, b] as raw 0–255 sRGB bytes. */
...
/** Alpha stays 0 so the still areas fall through to the element's CSS colour. */
function makeBgColor(hex: string): THREE.Vector4 {
  const [r, g, b] = hexToBytes(hex);
  return new THREE.Vector4(r / 255, g / 255, b / 255, 0);
}
...
class CommonGL {
  ...
  init(container: HTMLElement) {
    ...
    // Timer replaces the deprecated Clock; connect() uses the Page Visibility
    // API so a backgrounded tab doesn't return with a huge delta.
...
export interface PixelLiquidBgProps {
  ...
  palette?: string[];
  /** colour of the still areas; kept transparent so the CSS background shows */
  bgColor?: string;
...
export default function PixelLiquidBg({
  palette: paletteStops = ["#000000", "#2a0020", "#8c0f60", "#e8227a", "#ff85b3"],
  bgColor = "#000000",
  pixelSize = 18,
  resolution = 0.4,
  mouseForce = 8,
  ...
  useEffect(() => {
    ...
    const outputUniforms: Uniforms = {
      ...
      bgColor: { value: makeBgColor(bgColor) },
      uTime: { value: 0 },
    ...
    // The sim only runs while the element is both on screen and in a visible
    // tab — a page can hold several of these, and each one is a full fluid solve.
    const sync = () => {
      const should = pageVisible && onScreen;
    ...
    return () => {
      ...
      // Release the WebGL context outright — browsers cap how many stay live,
      // and HMR/StrictMode remount this effect repeatedly in dev.
